...
from .events import (
    AudioTranscribingDone,
    AudioTranscribingStart,
    ComposeDone,
    MessageComposeEvent,
)
...
class DefaultAgentMessageComposer(AgentMessageComposerABC):
    """The aiogram to Agent message composer."""
    ...
    async def stream_compose(
        ...
        message_id = f"msg_id:{message.message_id}"
        ...
        need_transcribing: bool = bool(message.voice or message.audio)

        if need_transcribing:
            yield MessageComposeEvent(data=AudioTranscribingStart())
        ...
        if need_transcribing:
            yield MessageComposeEvent(
                data=AudioTranscribingDone(
                    transcribed=voice_msg or audio_msg,
                ),
            )
